...
| v0.4.0 final | frozen | — |
| v0.6.0 | no | Additive only. Adds run bundles (`bundle`, `verify-run`) and OpenTelemetry retrieval attributes (`search --otel`). Neither touches the container: a run bundle is a JSON envelope over existing `annpack-receipt-v2` receipts and defines no cryptography, and the attributes define names only. No section type, manifest field, or index format changed, and no artifact root changed. The conformance contract is unchanged at four verbs. |
